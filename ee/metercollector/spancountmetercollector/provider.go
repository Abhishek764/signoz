// Package spancountmetercollector emits the signoz.meter.span.count meter:
// the count of spans ingested per (workspace, retention) bucket.
//
// The query body in this file is inlined on purpose. Per the meterreporter
// duplication policy, no helper is shared with the size sibling even though
...
// Collect aggregates span-count samples for the request window, broken out
...
// builder: billing shape changes for span.count must never bleed into
// span.size.
