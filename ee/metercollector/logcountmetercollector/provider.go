// Package logcountmetercollector emits the signoz.meter.log.count meter:
// the count of log records ingested per (workspace, retention) bucket.
//
// The query body in this file is inlined on purpose. Per the meterreporter
// duplication policy, no helper is shared with the size sibling — even
// though the two queries look similar — so a future change to one meter's
// billing shape can never silently change the other.
...
// referencing the symbol (not a string) makes typos compile errors instead
// of silent unbilled rows at Zeus.
var (
...
// Collect aggregates log-count samples for the request window, broken out
// by retention bucket. Returns at least one zero-valued sentinel meter on
...
// buildQuery is inlined per the duplication policy. Even though the size
// sibling produces a structurally similar query, do NOT factor a shared
// builder — billing shape changes for log.count must never bleed into
// log.size.
