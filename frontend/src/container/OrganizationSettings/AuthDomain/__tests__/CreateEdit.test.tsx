...
describe('CreateEdit Modal', () => {
	...
	describe('Provider Selection (Create Mode)', () => {
		...
		it('returns to provider selection when back button is clicked', async () => {
			...
			await expect(screen.findByText(/edit google authentication/i)).resolves.toBeInTheDocument();

			const backButton = screen.getByRole('button', { name: /back/i });
			fireEvent.click(backButton);

			await expect(screen.findByText(/configure authentication method/i)).resolves.toBeInTheDocument();
		});
	});
