// Package logsizemetercollector emits the signoz.meter.log.size meter:
// the size of log records ingested per (workspace, retention) bucket.
//
...
// Collect aggregates log-size samples for the request window, broken out
...
// builder: billing shape changes for log.size must never bleed into log.count.
