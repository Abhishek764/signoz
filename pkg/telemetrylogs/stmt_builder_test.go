...
func TestStmtBuilderBodyField(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			fm := NewFieldMapper(querybuilder.Options{BodyJSONEnabled: c.enableBodyJSONQuery})
			cb := NewConditionBuilder(fm, querybuilder.Options{BodyJSONEnabled: c.enableBodyJSONQuery})
			// build the key map
			mockMetadataStore := telemetrytypestest.NewMockMetadataStore()
			...
			aggExprRewriter := querybuilder.NewAggExprRewriter(instrumentationtest.New().ToProviderSettings(), nil, fm, cb, nil, querybuilder.Options{BodyJSONEnabled: c.enableBodyJSONQuery})
			statementBuilder := NewLogQueryStatementBuilder(
				instrumentationtest.New().ToProviderSettings(),
				mockMetadataStore,
				fm,
				...
				DefaultFullTextColumn,
				GetBodyJSONKey,
				querybuilder.Options{BodyJSONEnabled: c.enableBodyJSONQuery},
			)
