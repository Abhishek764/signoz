...
describe('OnboardingQuestionaire Component', () => {
	...
	describe('Step 1: Organization Details', () => {
		...
		it('shows other tool input when Others is selected', async () => {
			...
			await expect(screen.findByPlaceholderText(/what tool do you currently use/i)).resolves.toBeInTheDocument();
		});
		...
		it('shows migration timeline options only when specific observability tools are selected', async () => {
			...
			await expect(screen.findByText(/What is your timeline for migrating to SigNoz/i)).resolves.toBeInTheDocument();
	...
	describe('Step 2: About SigNoz', () => {
		it('renders about signoz questions after step 1 completion', async () => {
			...
			await expect(screen.findByText(/set up your workspace/i, {})).resolves.toBeInTheDocument();
			await expect(screen.findByText(/how did you first come across signoz/i, {})).resolves.toBeInTheDocument();
		});
		...
		it('enables next button when all fields are filled', async () => {
			...
			await expect(screen.findByPlaceholderText(/e\.g\., googling/i, {})).resolves.toBeInTheDocument();
		...
		it('shows other interest input when Others checkbox is selected', async () => {
			...
			await expect(screen.findByText(/what got you interested in signoz/i, {})).resolves.toBeInTheDocument();

			const othersCheckbox = screen.getByLabelText(/^others$/i);
			await user.click(othersCheckbox);

			await expect(screen.findByPlaceholderText(
	...
	describe('Step 3: Optimize SigNoz Needs', () => {
		...
		it('shows do later button', async () => {
			...
			await expect(screen.findByRole('button', { name: /i'll do this later/i })).resolves.toBeInTheDocument();
		});
	});
});
