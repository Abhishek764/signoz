// Package spansizemetercollector emits the signoz.meter.span.size meter:
// the size of spans ingested per (workspace, retention) bucket.
//
...
// Collect aggregates span-size samples for the request window, broken out
...
// builder: billing shape changes for span.size must never bleed into
// span.count.
