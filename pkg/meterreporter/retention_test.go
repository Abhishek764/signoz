...
func TestBuildLogsRetentionSlicesFromRowsEmptyWindowReturnsNil(t *testing.T) {
	t.Parallel()

	slices, err := buildLogsRetentionSlicesFromRows(nil, 100, 100)
	if err != nil {
...
func TestBuildLogsRetentionSlicesFromRowsNoRowsFallsBackToDefault(t *testing.T) {
	t.Parallel()

	startMs := time.Date(2026, 4, 28, 0, 0, 0, 0, time.UTC).UnixMilli()
	endMs := time.Date(2026, 4, 29, 0, 0, 0, 0, time.UTC).UnixMilli()

	slices, err := buildLogsRetentionSlicesFromRows(nil, startMs, endMs)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(slices) != 1 {
	...
	if slices[0].DefaultDays != types.DefaultRetentionDays {
		t.Fatalf("DefaultDays = %d, want %d", slices[0].DefaultDays, types.DefaultRetentionDays)
	}
...
func TestBuildLogsRetentionSlicesFromRowsActiveAtStart(t *testing.T) {
	t.Parallel()
	...
	preWindow := makeRow(time.Date(2026, 4, 24, 15, 13, 15, 0, time.UTC), 15, condition)

	slices, err := buildLogsRetentionSlicesFromRows([]*types.TTLSetting{preWindow}, startMs, endMs)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(slices) != 1 {
	...
	if slices[0].DefaultDays != 15 {
		t.Fatalf("DefaultDays = %d, want 15", slices[0].DefaultDays)
	}
...
func TestBuildLogsRetentionSlicesFromRowsMidWindowChangeProducesTwoSlices(t *testing.T) {
	t.Parallel()
	...
	preWindow := makeRow(time.Date(2026, 4, 21, 15, 28, 3, 0, time.UTC), 30, "")
	inWindow := makeRow(changeAt, 15, `[{"conditions":[{"key":"signoz.workspace.key.id","values":["a"]}],"ttlDays":90}]`)

	slices, err := buildLogsRetentionSlicesFromRows([]*types.TTLSetting{preWindow, inWindow}, startMs, endMs)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(slices) != 2 {
	...
	if slices[1].DefaultDays != 15 || len(slices[1].Rules) != 1 || slices[1].Rules[0].TTLDays != 90 {
		t.Fatalf("second slice config = (%d, %#v), want (15, one rule at 90)", slices[1].DefaultDays, slices[1].Rules)
	}
}

func TestBuildLogsRetentionSlicesFromRowsThreeChangesInOneWindow(t *testing.T) {
	t.Parallel()
	...
	slices, err := buildLogsRetentionSlicesFromRows(rows, startMs, endMs)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(slices) != 4 {
	...
	wantDefaults := []int{types.DefaultRetentionDays, 90, 365, 90}
...
func TestBuildLogsRetentionSlicesFromRowsRowAtExactStartIsActiveConfig(t *testing.T) {
	t.Parallel()
	...
	row := makeRow(time.UnixMilli(startMs).UTC(), 90, "")

	slices, err := buildLogsRetentionSlicesFromRows([]*types.TTLSetting{row}, startMs, endMs)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(slices) != 1 {
...
func TestBuildLogsRetentionSlicesFromRowsRowAtOrAfterEndIsIgnored(t *testing.T) {
	t.Parallel()

	startMs := time.Date(2026, 4, 28, 0, 0, 0, 0, time.UTC).UnixMilli()
	endMs := time.Date(2026, 4, 29, 0, 0, 0, 0, time.UTC).UnixMilli()

	preWindow := makeRow(time.Date(2026, 4, 21, 0, 0, 0, 0, time.UTC), 30, "")
	atEnd := makeRow(time.UnixMilli(endMs).UTC(), 90, "")
	afterEnd := makeRow(time.UnixMilli(endMs+1).UTC(), 365, "")

	slices, err := buildLogsRetentionSlicesFromRows([]*types.TTLSetting{preWindow, atEnd, afterEnd}, startMs, endMs)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(slices) != 1 {
...
func TestBuildLogsRetentionSlicesFromRowsParsesConditionJSON(t *testing.T) {
	t.Parallel()
	...
	row := makeRow(time.Date(2026, 4, 24, 15, 13, 15, 0, time.UTC), 15, condition)
