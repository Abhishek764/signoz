...
describe('TimeSeries', () => {
	...
	it('warning tooltip shows metric details link', async () => {
		...
		await expect(screen.findByText('metric details')).resolves.toBeInTheDocument();
	});
	...
	it('shows save unit prompt with enabled button when metric has no unit and a unit is selected', async () => {
		...
		await expect(screen.findByText('Set the selected unit as the metric unit?')).resolves.toBeInTheDocument();
